...
Matrix<T> Transporation(Matrix<T> A) {
    ...
    std::vector<T> line(A.get_coloumns_number(), 0);
    std::vector<T> lines(A.get_lines_number(), line);
...
int main() {
    ...
    std::cout << a << "\n" << b << "\n" << a*b;
    std::cout << a + b << "\n" << a - b;
    std::cout << (a == 1) << ' ' << (a == b) << " " << (1 == a);
    a = b;
    std::cout << "\n" << a << "\n";
    std::cout << InverseM(Matrix<int>::E_matrix(3));
    return 0;
}
